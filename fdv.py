...
TITLE = "🔥 Outil FDV by HARDCORE — v0.7 🔥"
...
RACES = {
    "humains": {
        ...
        "buildings": [
            ("🏘️", "Secteur résidentiel", "pop"),
            ("🌿", "Ferme biosphérique", "food"),
            ("🎓", "Académie des sciences (T1→T2)", "t1t2"),
            ("🧠", "Centre de neurocalibrage (T2→T3)", "t2t3"),
            ("🥫", "Réserve alimentaire", "bonus"),
            ("🏢", "Tour d’habitation", "bonus"),
            ("🧬", "Laboratoire de biotechnologie", "bonus"),
        ],
        "levels": [
            [22, 20, 0, 0, 0, 0, 0],
            [23, 22, 0, 0, 0, 0, 0],
    ...
    "rocktal": {
        ...
        "buildings": [
            ("🪨", "Enclave stoïque", "pop"),
            ("💎", "Culture du cristal", "food"),
            ("🧿", "Forge runique (T1→T2)", "t1t2"),
            ("🗿", "Orictorium (T2→T3)", "t2t3"),
            ("🧱", "Monolithe", "bonus"),
            ("🧪", "Centre de recherche sur les minéraux", "bonus"),
        ],
        "levels": [
            [21, 21, 0, 0, 0, 0],
            [23, 23, 0, 0, 0, 0],
    ...
    "mecas": {
        ...
        "buildings": [
            ("🏭", "Chaîne de production", "pop"),
            ("⚗️", "Usine de fusion de cellules", "food"),
            ("📡", "Réseau d’actualisation (T1→T2)", "t1t2"),
            ("🧮", "Centre d’informatique quantique (T2→T3)", "t2t3"),
            ("💾", "Chaîne de production de micropuces", "bonus"),
            ("🦾", "Centre d’assemblage automatisé", "bonus"),
        ],
        "levels": [
            [17, 20, 0, 0, 0, 0],
            [19, 21, 0, 0, 0, 0],
    ...
    "kaelesh": {
        ...
        "buildings": [
            ("🛖", "Refugium", "pop"),
            ("⚛️", "Condensateur d’antimatière", "food"),
            ("🧿", "Salles de réalisation (T1→T2)", "t1t2"),
            ("🌌", "Forum de la transcendance (T2→T3)", "t2t3"),
            ("🧲", "Convecteur d’antimatière", "bonus"),
            ("🪺", "Accélérateur par chrysalide", "bonus"),
            ("🔮", "Modulateur psionique", "bonus"),
        ],
        "levels": [
            [20, 20, 0, 0, 0, 0, 0],
            [21, 22, 0, 0, 0, 0, 0],
...
def strip_invisible_emoji(s: str) -> str:
    out = []
    for ch in s:
        cp = ord(ch)
        if 0xFE00 <= cp <= 0xFE0F:
...
def display_width(s: str) -> int:
    plain = strip_invisible_emoji(strip_ansi(s))
    w = 0
    for ch in plain:
        cp = ord(ch)
...
def render_min_levels(race_key: str, slot: int) -> str:
    ...
    rows = []
    for (icon, name, _role), lvl in zip(cfg["buildings"], req):
...
def render_delta_table(race_key: str, slot: int, current: Sequence[int]) -> str:
    ...
    rows = []
    for (icon, name, _role), c, r, d in zip(cfg["buildings"], current, req, delta):
    ...
    role_order = {"pop": 0, "food": 0, "t1t2": 1, "t2t3": 2, "bonus": 3}
    prio = sorted(
    ...
    if not prio:
        ...
        for i, ((name, _role), d) in enumerate(prio, 1):
            lines.append(f"{i:>2}. {name} (+{d})")
    return "\n".join(lines)


def find_max_slot_from_current(race_key: str, current: Sequence[int]) -> int:
    max_slot = 0
    for slot in range(1, 19):
        req = required_levels(race_key, slot)
        if all(c >= r for c, r in zip(current, req)):
            max_slot = slot
        else:
            break
    return max_slot
...
def render_auto_slot_result(race_key: str, current: Sequence[int]) -> str:
    max_slot = find_max_slot_from_current(race_key, current)
    lines = [f"{ANSI_BOLD}Mode AUTO-SLOT{ANSI_RESET}"]
    if max_slot == 0:
        lines.append("Slot max atteignable: 0")
        lines.append(f"Prochain slot: 1 ({level_label_from_slot(1)}) | Palier: {format_int(POP_THRESHOLDS[0])}")
        next_slot = 1
    else:
        lines.append(
            f"Slot max atteignable: {max_slot} ({level_label_from_slot(max_slot)}) | "
            f"Palier: {format_int(POP_THRESHOLDS[max_slot - 1])}"
        )
        next_slot = max_slot + 1

    if next_slot <= 18:
        lines.append(f"Prochain slot: {next_slot} ({level_label_from_slot(next_slot)})")
        lines.append(render_delta_table(race_key, next_slot, current))
    else:
        lines.append("🎉 Tous les slots (jusqu’à 18) sont atteints.")
    return "\n".join(lines)
...
def render_full_level_block(race_key: str, tier: int) -> str:
    ...
    if width >= estimated:
        ...
        rows = []
        for bidx, (icon, name, _role) in enumerate(cfg["buildings"]):
    ...
    for i, s in enumerate(slots):
        req = cfg["levels"][s - 1]
        r_rows = []
        for (icon, name, _role), lv in zip(cfg["buildings"], req):
...
def copy_to_clipboard_windows(text: str) -> bool:
    if os.name != "nt":
        return False
    try:
        proc = subprocess.run(["clip"], input=text.encode("utf-16le"), check=True)
...
def interactive_export(content: str) -> None:
    choice = ask("\nExporter ? (1=copy 2=save 3=non): ").lower()
    if choice == "1":
        if copy_to_clipboard_windows(content):
            print("📋 Copié dans le presse-papiers Windows.")
        else:
            print("⚠️ Copie non disponible (Windows/clip requis).")
    elif choice == "2":
        filename = ask("Nom du fichier (ex: fdv_export.txt): ")
        path = save_txt(content, filename)
        print(f"💾 Export sauvegardé: {path}")
...
def interactive_flow() -> int:
    print(render_header())
    print("Aide: slot en format 18 ou 3.6 | quitter: q")
    ...
    output_blocks = []
    output_blocks.append(render_min_levels(race_key, slot))
    ...
    if full in {"1", "2", "3"}:
        print()
        full_txt = render_full_level_block(race_key, int(full))
        output_blocks.append(full_txt)
        print(full_txt)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Outil FDV by HARDCORE — v0.7")
    ...
    p.add_argument("--save", metavar="FILE", help="Sauvegarder la sortie dans un .txt")
    p.add_argument("--copy", action="store_true", help="Copier la sortie dans le presse-papiers Windows")
    return p
...
def main() -> int:
    ...
    try:
        ...
        output_blocks = [render_header(), "", render_min_levels(race_key, slot)]
        print(output_blocks[0])
        print()
        print(output_blocks[2])
        ...
        if args.full_level:
            print()
            full_txt = render_full_level_block(race_key, int(args.full_level))
            output_blocks.append("")
            output_blocks.append(full_txt)
            print(full_txt)
        ...
        final_content = "\n".join(output_blocks)
        if args.save:
            path = save_txt(final_content, args.save)
            print(f"\n💾 Export sauvegardé: {path}")
        if args.copy:
            ok = copy_to_clipboard_windows(final_content)
            if ok:
                print("\n📋 Copié dans le presse-papiers Windows.")
            else:
                print("\n⚠️ Copie non disponible (Windows/clip requis).")
